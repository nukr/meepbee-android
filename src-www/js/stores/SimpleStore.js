...
Object.assign(Store, EventEmitter.prototype, {
  ...
  turnOn(animation) {
    let ani = State.animation
    for (let key in ani) {
      key === animation ? ani[key] = true : ani[key] = false
    }
  },
...
Store.dispatchToken = AppDispatcher.register(function eventHandlers(evt) {
  ...
  switch (action.actionType) {
    ...
    case AppConstants.ANIMATION:
      action.items === 'stop' ? Store.turnOff() : Store.turnOn(action.items)
      Store.emit(AppConstants.CHANGE_EVENT);
      break;

    case AppConstants.DATA_LOAD:
      State.products = action.items;
      Store.emit(AppConstants.CHANGE_EVENT);
      break;
